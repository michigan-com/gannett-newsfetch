...
*/
func GetArticlesByDay(siteCode string, date time.Time, gannettAPIKey string) []*m.SearchArticle {
	...
	return getArticles(siteCode, startDate, endDate, gannettAPIKey)
}
...
*/
func getArticles(siteCode string, startDate string, endDate string, gannettAPIKey string) []*m.SearchArticle {
	var articleResponse *m.ArticlesResponse = &m.ArticlesResponse{}
	var queryParams = GetDefaultSearchValues(siteCode, gannettAPIKey)
