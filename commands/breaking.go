...
func FetchBreakingNews(session *mgo.Session, siteCodes []string, gnapiDomain string, loopInterval time.Duration, gannettSearchAPIKey string) {
	for {
		...
		for _, siteCode := range siteCodes {
			...
			go func(siteCode string) {
				...
				articles := api.GetBreakingNews(siteCode, gannettSearchAPIKey)
